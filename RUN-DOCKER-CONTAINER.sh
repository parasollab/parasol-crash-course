...
case "$3" in
  ...
  case "$2" in
    ...
    ;;
    ( * )
    docker exec -i -t ${CONTAINER} bash -i -c "~/ppl_cc_ws/docker/ppl-crash-course/scripts/run-command-repeatedly.sh $2"
  esac
  ;;
  ( *".launch")
  docker exec -i -t ${CONTAINER} bash -i -c "~/ppl_cc_ws/docker/ppl-crash-course/scripts/run-ros2-launch-repeatedly.sh $2 $3"
  ;;
  ( * )
